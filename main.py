...
#       print(ch_name)
#        category_id = 754241560597495848
...
async def on_message(message):
    ...
    if True == message.content.startswith('!copy'):
        ...
        finish_text = f"{student_name}さん、techmeetsを受講いただきありがとうございました。\n\n"\
            "本日をもちまして、サービスの方を終了させていただきます。\n\n"\
            "今後ですが、techmeetsのコンテンツを確認及びご利用することができなくなります。\n\n"\
        ...
        anketo_text = f'{student_name}さん\n以前はtechmeetsをご利用いただきまして誠にありがとうございました。\nその後いかがお過ごしでしょうか。\n\ntechmeetsでは卒業生を対象にフォローアップアンケートを実施しております。\n差し支えの無い範囲で構いませんので、回答のご協力を何卒よろしくお願い申し上げます。\nhttps://forms.gle/KL4vX9fvGYkXpSVy9 \n\nお忙しい中お読みいただきまして誠にありがとうございます。\n皆様の今後のご活躍をお祈り申し上げます。'
        await message.channel.send(anketo_text)
    elif True == message.content.startswith('!custom'):
        custom_text = f"※このメッセージは今月末で受講期間が終了する方に向けて送信しています。\n"\
            "万が一受講期間がずれていた場合はご連絡いただけると幸いです。\n\n"\
            "【{student_name}】さん、平素よりTechmeetsをご利用いただきありがとうございます。\n\n"\
            "受講期間が残すところ1週間となりましたことをご連絡致します。\n\n"\
            "本サービスは月末まで利用可能です。\n"\
            "月末の翌営業日(月初め)に受講終了通知とともにサービスがご利用できなくなりますのでご承知おきください。\n\n"\
            "また、お手数をおかけしますが、今後私たちのサービス改善のために下記リンクからアンケートのご協力をお願いします。\n"\
            "https://forms.gle/5LJ35xqgxzohn61W6 \n\n"\
            "それでは残りの受講期間もどうぞよろしくお願い申し上げます。\n"\
        .format(student_name=student_name)
        await message.channel.send(custom_text)
